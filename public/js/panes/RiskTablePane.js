...
import { loadRiskDefinitions, loadRiskTable } from "../core/riskData.js";
...
function initRiskTablePane(host, settings, api) {
  ...
  Promise.all([
    loadRiskTable(riskKey),
    loadRiskDefinitions()
  ]).then(([rows, definitionsData]) => {
    if (!rows || rows.length === 0) {
      renderHostMessage(host, 'No rows found for "' + riskKey + '".', "rt-error", false);
      return;
    }
